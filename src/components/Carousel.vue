    ...
    moveStack(dragXPos) {
      ...
      this.stack = this.stack.map((card, index) => {
        ...
        const xPos = isActiveCard
          ...
          : this.cardDefaults[index].xPos + (this.xPosOffset / (this.cardWidth + this.paddingHorizontal)) * activeCardOffset;

        const scale = isActiveCard
          ? this.cardDefaults[index].scale
          : this.cardDefaults[index].scale + (this._scaleMultiplier / (this.cardWidth + this.paddingHorizontal)) * activeCardOffset;
